...
% Vertically Slipping Fault: Continual Slip Case
...
erate0 = 0.04; %initial incision rate of channels at boundary
...
%Variable Arrays
    ...
    %Continually Slipping Fault
    faultposition = 10; %location of the fault in x
    faultmotion(N:1) = zeros(N:1); %set up a fault array for all x positions
    faultmotion(1:faultposition) = 0.15; % the plate rises according to this rate from 1 to the fault position
    faultmotion(faultposition+1:N) = 0; %the plate sinks from the fault position to N
...
for i = 1:imax
        ...
        weatherrate(1:N) = min(A1.*exp(-Hinitial(1:N)/hstar),A0 + b.*Hinitial(1:N)); %weather rock according to humped profile
        Hbedrock(1:N) = Hbedrock(1:N) - (rhorock/rhosoil)*weatherrate(1:N)*dt+faultmotion(1:N);%update bedrock position with weathering and fault motion
        Hinitial(1:N) = (rhorock/rhosoil)*weatherrate(1:N)*dt + Hinitial(1:N);%update soil thickness from weathering
        H(1:N) = Hbedrock(1:N) + Hinitial(1:N); %update total height of profile
        ...
        dh(1:N) = [-erate0 dhint -erate0]; %carry away material according to the channel incision rate at the two boundaries
    ...
    fill(uu,rr,[.82,.82,.965]) %fill the polygon created
    
    %Color the bedrock
    xx = [x,x];        % repeat x values
    yy = [Hbedrock,bottomline];   % vector of upper & lower boundaries
    fill(xx,yy,[.349,.176,.525]) %fill the polygon created
